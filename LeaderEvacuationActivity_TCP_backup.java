...
public class LeaderEvacuationActivity_TCP_backup extends FragmentActivity {
...
*/
			...
			dialogButtonRoute.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					dialog.dismiss();
		      		...
		      		Dialog prcoseedialog = ProgressDialog.show(LeaderEvacuationActivity_TCP_backup.this, "",
							"Path Finding...", true);
				    ...
				    try{
							...
							String result=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=findsegmentsbyleaderid&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);					
					        ...
					        if(result.equals("")){
					        		...
					        		mapoperationtype=0;	
									String result1=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=findshelterbyleaderid&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);  
				    		        if(result1.equals(""))
				    		        	pushedroutedetail="";					    		        
				    		        else
				...
				*/	
					...
					if(mapoperationtype==0){
			      		...
			      		Toast.makeText(LeaderEvacuationActivity_TCP_backup.this,
					...
					if(mapoperationtype==1){
			      		...
			      		Toast.makeText(LeaderEvacuationActivity_TCP_backup.this,
								...
								try {
									...
									String closestjunctionresult=cc.socketservice("http://"+tip+":"+tport+"/ga/closestjunction?&geometry="+envelopcoor+"&geometryType=esriGeometryEnvelope"
											...
											+ "&returnM=false&returnDistinctValues=false&f=pjson&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);												   															
								   double dis=999999;
								   double startx=0,starty=0;
									...
									String polylinebarriers=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=showaffectedbarriers&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport); 									   												 
									
									String result=cc.socketservice("http://"+tip+":"+tport+"/ga/nacloseshelter?incidents="+incidentpt+"&facilities="+shelters+
												"&polylineBarriers="+polylinebarriers+"&travelDirection=esriNATravelDirectionToFacility" +
										   		...
										   		"&directionsLengthUnits=esriNAUMiles&timeOfDayUsage=esriNATimeOfDayUseAsStartTime&timeOfDayIsUTC=false&returnZ=false&f=pjson&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);												   															
					...
					//2. route
			      		...
			      		Toast.makeText(LeaderEvacuationActivity_TCP_backup.this,
								...
								try {
									...
									String result0=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=findshelterbyleaderid&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);  
									...
									String result=cc.socketservice("http://"+tip+":"+tport+"/ga/naroute?stops="+incidentpt+
												"&polylineBarriers="+polylinebarriers+"&travelDirection=esriNATravelDirectionToFacility" +
										   		"&defaultTargetFacilityCount=1&outSR=102100&impedanceAttributeName=Length" +
										   		"&restrictUTurns=esriNFSBAllowBacktrack&useHierarchy=false&returnDirections=true&returnCFRoutes=true&returnFacilities=false&returnIncidents=false&returnBarriers=false" +
										   		"&returnPolylineBarriers=false&returnPolygonBarriers=false&directionsLanguage=en&directionsOutputType=esriDOTComplete" +
					              ...
					              if (queryResults.getGraphics().length > 0) {								             
					            	...
					            	Graphic newGraphic = new Graphic(null, null, attributes);
					              	...
					              	mTempSheltersLayer.applyEdits(null, null,new Graphic[] {newGraphic}, new CallbackListener<FeatureEditResult[][]>() {
					              		...
					              		@Override
					              		public void onError(Throwable e){ 
					              			 Log.d(LeaderEvacuationActivity_TCP_backup.TAG, "error updating feature: " + e.getLocalizedMessage());
					              		...
					              		public void onCallback(FeatureEditResult[][] result) { 
					              	        ...
					              	        if (result[2] != null && result[2][0] != null && result[2][0].isSuccess()) {
					              	          ...
					              	          Log.d(LeaderEvacuationActivity_TCP_backup.TAG, "Success updating feature with id=" + result[2][0].getObjectId());
	...
	InputData=new DataStruct();	 	
   	...
   	//get pi  ip info   	
    gatewayip=cc.getWIFIIp("http://"+tip+":"+tport+"/ga/keyvaluestore?action=showallleadersegment&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);    
    if(gatewayip!="")
    	Label.setText(piip+"#"+gatewayip);
    ...
    map.setOnTouchListener(new MyTouchListener(LeaderEvacuationActivity_TCP_backup.this, map));
		...
		dialogButtonSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				//******************
				    ...
				    String result2;
					try {						
						//get  leaders affected by the risk shelter 
						result2 = cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=getleadersbyshelterid&shelterid="+shelterid+"&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);
				    ...
				    if(result2.equals("")){
		  		        	...
		  		        	for(int j=0;j<temp.length;j++){
		  		        		if(temp[j]!="")
		  		        		{
		  		        			String tleaderinfos=temp[j];
		  		        			String[] tleaderinfo = tleaderinfos.split(",");
		  		        			String tleaderid2=tleaderinfo[0]+","+tleaderinfo[1]+","+tleaderinfo[2]+","+tleaderinfo[3];
		  		        			

								   //send warning to the affected leader     
								   cc.socketservice("http://"+tip+":"+tport+"/ga/communication?action=notificationleadersbyshelterid&shelterid="+shelterid+"&str=&leaderid="+tleaderid2,tsocketaddress,tsocketport);						   										   												
				...
				int roadsegmentid=0; 
				try {
				   String result=cc.socketservice("http://"+tip+":"+tport+"/ga/roadsegment?where=1%3D1&geometry={\"paths\":[["+tempaffectedstr+"]],\"spatialReference\":{\"wkid\":4326}}" +
					   		...
					   		+ "&returnIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&returnZ=false&returnM=false&f=pjson&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);
				   if(result!=""){
						    ...
						    for (int i = 0; i < jsonArray.length(); i++) {
						        ...
						        roadsegmentid= jsonObjectattribute.getInt("OBJECTID");	
								   ...
								   if (roadsegmentid>0){
									   String result0=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=findsegmentsbysegmentid&segmentid="+roadsegmentid+"&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);
									   
									   //when segment is report risk, double report will change the risk to safety
									   if(result0.equals("true")){
									   
										   cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=removeaffectsegment&segmentid="+roadsegmentid+"&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);	
									   }
									   else{
										   
									   		msg = "6;3"+leaderid+";200;#;"+type+":"+String.valueOf(roadsegmentid)+":"+affectedstr;	
										   ...
										   /*
									   		...
									   		cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=addaffectedsegment&segmentid="+roadsegmentid+"&affectedcoor="+affectedstr+"&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);	
									   	
									   		String result2=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=getleadersbysegmentid&segmentid="+roadsegmentid+"&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);	
									   		...
									   		if(result2.equals("")){
							  		        	...
							  		        	for(int j=0;j<temp.length;j++){
							  		        		if(temp[j]!="")
							  		        		{
													    ...
													    result3 = cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=findshelterbyleaderid&leaderid="+tleaderid2,tsocketaddress,tsocketport);
														
														//notification from CC to pi to leader
													        
														//String result4=cc.socketservice("http://"+tip+":"+tport+"/ga/communication?action=notificationpiandleadersbysegmentid&segmentid="+roadsegmentid+"&str="+result3+"&leaderid="+tleaderid2,tsocketaddress,tsocketport);
														
														cc.socketservice("http://"+tip+":"+tport+"/ga/communication?action=udpunicastdetour&segmentid="+roadsegmentid+"&str="+result3+"&leaderid="+tleaderid2,tsocketaddress,tsocketport);
   ...
   */
  public void udpreport(String msg){
	   ...
	   try {
			...
			DatagramPacket packet = new DatagramPacket(buffer, buffer.length, InetAddress.getByName("239.1.1.171"), 7385);
  ...
  {  
      ...
      {  
			...
			new AlertDialog.Builder(LeaderEvacuationActivity_TCP_backup.this)
			.setTitle("Exit?")
			...
			public void onClick(View v) {
				...
				try{
					if (v == StartButton) {						
						...
						//Properties prop1 = PropertiesUtil.getProperties();
						pushedroutedetail = (String) prop1.get("defaultroute");	
						//udpmulticastwarningtofollower(pushedroutedetail);
	 ...
	 private void initializeRoutingAndGeocoding() {
		...
		void updaterouteUI() {
			...
			if (mResults == null ||mResults.getRoutes().size()==0) {
				Toast.makeText(LeaderEvacuationActivity_TCP_backup.this, mException.toString(),
						Toast.LENGTH_LONG).show();
				return;
			}
		...
		if (mResults == null ||mResults.getRoutes().size()==0) {
			Toast.makeText(LeaderEvacuationActivity_TCP_backup.this, "No road available, be careful.",
					Toast.LENGTH_LONG).show();	
			showdefaultroute(0);
			showbarriers(1);
			return;
		...
		if(mResults.getRoutes().size()>1){
			...
			//get start segment id
			 ...
			 tstartpath="["+Double.toString(t_start_pt_11.getX())+","+Double.toString(t_start_pt_11.getY())+"],["+Double.toString(t_start_pt_21.getX())+","+Double.toString(t_start_pt_21.getY())+"]";
			   ...
			   try {
				   ...
				   String result=cc.socketservice("http://"+tip+":"+tport+"/ga/roadsegment?where=1%3D1&geometry={\"paths\":[["+tstartpath+"]],\"spatialReference\":{\"wkid\":4326}}" +
			 ...
			 //get middle segment ids
			   ...
			   try {
				   ...
				   String result=cc.socketservice("http://"+tip+":"+tport+"/ga/roadsegment?where=1%3D1&geometry={\"paths\":[["+tpath+"]],\"spatialReference\":{\"wkid\":4326}}" +
			 ...
			 //get end segment id
				 ...
				 tendpath="["+Double.toString(t_start_pt_31.getX())+","+Double.toString(t_start_pt_31.getY())+"],["+Double.toString(t_start_pt_41.getX())+","+Double.toString(t_start_pt_41.getY())+"]";
				   ...
				   try {
					   ...
					   String result=cc.socketservice("http://"+tip+":"+tport+"/ga/roadsegment?where=1%3D1&geometry={\"paths\":[["+tendpath+"]],\"spatialReference\":{\"wkid\":4326}}" +
						   		"&geometryType=esriGeometryPolyline&inSR=&spatialRel=esriSpatialRelWithin&outFields=OBJECTID&returnGeometry=true&returnDistinctValues=false"
						   		+ "&returnIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&returnZ=false&returnM=false&f=pjson&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);
					   if(result!=""){	
						   	  JSONObject jsonObject = new JSONObject(result);	     
					   ...
					   //String result1=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=removeleaderbyleaderid&leaderid="+shelterid+","+regionid+","+leaderid,tsocketaddress,tsocketport);
					   
					   cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=removeleaderbyleaderidandsegmentorder&affectedsegmentorder="+affectedsegmentorder+"&leaderid="+shelterid+","+regionid+","+leaderid,tsocketaddress,tsocketport);
			...
			//query shelter id
				   ...
				   try {				   													   							
						/*
					   String result=cc.socketservice("http://"+tip+":"+tport+"/ga/shelters?where=&geometry="+(shelterlocaiton.getX())+"%2C"+(shelterlocaiton.getY())+
					    		...
					    		+ "&returnDistinctValues=false&returnIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&returnZ=false&returnM=false&f=pjson&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);
					   */
					   String result=cc.socketservice("http://"+tip+":"+tport+"/ga/shelters?where=&geometry="+ "%7B"+(shelterlocaiton.getX()-0.001)+"%2C"+(shelterlocaiton.getY()-0.001)+"%2C"+(shelterlocaiton.getX()+0.001)+"%2C"+(shelterlocaiton.getY()+0.001)+"%7D"+
				...
				//leader register to cc for the default route
				 ...
				 cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=addshelterbyleader&shelterid="+shelterid+"&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);   
				 //String result2=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=addleaderbysegment&segmentid="+segmentids+"&segmentstartpts="+segmentstartpts+"&leaderid="+shelterid+","+regionid+","+leaderid,tsocketaddress,tsocketport);
						 
				 cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=addleaderbysegmentorder&segmentid="+segmentids+"&affectedsegmentorder="+affectedsegmentorder+"&segmentstartpts="+segmentstartpts+"&leaderid="+shelterid+","+regionid+","+leaderid,tsocketaddress,tsocketport);
		...
		public void run() {
			if(msg!=null){
				...
				if(msg.split(";")[1].equals("1")){//fromat data = "13;1;0;199;1;###"		
					...
					final DialogInterface dialog=new AlertDialog.Builder(LeaderEvacuationActivity_TCP_backup.this)
					...
					try {
		                ...
		                if(affectedsegmentorder>=0){
						   ...
						   //shelter location from local config
						    ...
						    //shelter location from CC
						    /*
						    String result0=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=findshelterbyleaderid&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);  
							for (shelter st : shelters) {
								if(st.id==Integer.parseInt(result0)){
								//if(st.id==Integer.parseInt(leaderlocation)){	
									 sx=st.x;
									 sy=st.y;								
								}																							
							}
							*/
							...
							//polyline barriers or key value store						
							 polylinebarriers=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=showaffectedbarriers&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport); 							   												 
							 
							String result2=cc.socketservice("http://"+tip+":"+tport+"/ga/naroute?stops="+incidentpt+
										"&polylineBarriers="+polylinebarriers+"&travelDirection=esriNATravelDirectionToFacility" +
						...
						//***************************************
							...
							try {
								...
								//get the closest road junction use the leader's location								
								String envelopcoor=(leaderlocaiton.getX()-0.0005)+","+(leaderlocaiton.getY()-0.0005)+","+(leaderlocaiton.getX()+0.0005)+","+(leaderlocaiton.getY()+0.0005);
							   ...
							   double startx=0,starty=0;
								...
								//add regionid in shelter choose--04/06/2016
								String shelters="%7B%0D%0A++%22type%22+%3A+%22features%22%2C%0D%0A++%22url%22+%3A+%22http%3A%2F%2F"+tip+"%3A"+tport+"%2Fga%2Fshelters%3Fwhere%3DAVAILABLE%253D1%2Band%2BREGIONID%2B%253D"+regionid+"%26returnGeometry%3Dtrue%26outFields%3DName%26f%3Dpjson%22%0D%0A%7D";
								//polyline barriers or key value store
								//String polylinebarriers="%7B+%0D%0A++%22type%22+%3A+%22features%22%2C%0D%0A++%22url%22+%3A+%22http%3A%2F%2F"+tip+"%3A8080%2Fga%2Fpolylinebarriers%3Fwhere%3D1%253D1%26outFields%3DNAME%26returnGeometry%3Dtrue%26f%3Dpjson%22%0D%0A%7D";
								polylinebarriers=cc.socketservice("http://"+tip+":"+tport+"/ga/keyvaluestore?action=showaffectedbarriers&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport); 
								   												 
								//get at least 2 shelters/path for the affected leaders
								String result=cc.socketservice("http://"+tip+":"+tport+"/ga/nacloseshelter?incidents="+incidentpt+"&facilities="+shelters+
											"&polylineBarriers="+polylinebarriers+"&travelDirection=esriNATravelDirectionToFacility" +
									   		"&defaultTargetFacilityCount=2&outSR=102100&impedanceAttributeName=Length" +
									   		"&restrictUTurns=esriNFSBAllowBacktrack&useHierarchy=false&returnDirections=true&returnCFRoutes=true&returnFacilities=false&returnIncidents=false&returnBarriers=false" +
									   		"&returnPolylineBarriers=false&returnPolygonBarriers=false&directionsLanguage=en&directionsOutputType=esriDOTComplete" +
									   		"&outputLines=esriNAOutputLineTrueShapeWithMeasure&outputGeometryPrecisionUnits=esriDecimalDegrees" +
									   		"&directionsLengthUnits=esriNAUMiles&timeOfDayUsage=esriNATimeOfDayUseAsStartTime&timeOfDayIsUTC=false&returnZ=false&f=pjson&leaderid="+regionid+","+leaderid,tsocketaddress,tsocketport);												   																							   
