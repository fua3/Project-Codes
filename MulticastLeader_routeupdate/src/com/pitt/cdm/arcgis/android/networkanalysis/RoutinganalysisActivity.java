...
public class RoutinganalysisActivity extends Activity {
	MapView map = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		secureFeatureLayer = new ArcGISFeatureLayer("http://concepcion.gspia.pitt.edu:6080/arcgis/rest/services/GISSERVER/padangFacilitiesN/FeatureServer/0",
				MODE.ONDEMAND, creds);
		...
		try {
			mRouteTask = RouteTask
					.createOnlineRouteTask(
							"http://concepcion.gspia.pitt.edu:6080/arcgis/rest/services/GISSERVER/Padang_NAN/NAServer/Route",
							null);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		map.setOnLongPressListener(new OnLongPressListener() {
			...
			public boolean onLongPress(final float x, final float y) {
				...
				dialog = ProgressDialog.show(RoutinganalysisActivity.this, "",
				...
				Thread t = new Thread() {
					...
					public void run() {
						try {
							...
							Point e = (Point) GeometryEngine.project(loc, wm,
									egs);
							
							Point s = (Point) GeometryEngine.project(100.36815, -0.94309, SpatialReference.create(4326));
							// Create the stop points (start at our location, go
							// to pressed location)
							
							//not use GPS location in test
							//StopGraphic point1 = new StopGraphic(mLocation);
							
							StopGraphic point1 = new StopGraphic(s);
							
							StopGraphic point2 = new StopGraphic(e);
	...
	void updateUI() {
		dialog.dismiss();
		if (mResults == null) {
			Toast.makeText(RoutinganalysisActivity.this, mException.toString(),
