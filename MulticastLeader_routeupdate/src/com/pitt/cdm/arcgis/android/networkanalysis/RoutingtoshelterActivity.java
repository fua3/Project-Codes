...
import com.pitt.cdm.socket.broadcast.LeaderBroadcastActivity;
import com.pitt.cdm.arcgis.android.maps.R;

public class RoutingtoshelterActivity extends Activity {
	MapView map = null;
    ...
    String barrierUrl ="http://10.0.0.112:6080/arcgis/rest/services/AC/ACBarriers/FeatureServer/1/query?where=1%3D1&f=pjson";
	...
	public void onCreate(Bundle savedInstanceState) {
	  ...
	  //#########################
		...
		mTempSheltersLayer = new ArcGISFeatureLayer("http://10.0.0.112:6080/arcgis/rest/services/AC/ACFacilities/FeatureServer/0",
				ArcGISFeatureLayer.MODE.ONDEMAND);
		// add secure layer to the map
		map.addLayer(mTempSheltersLayer);
		
		//road segment layer
		mTemproadsegmentLayer= new ArcGISFeatureLayer("http://10.0.0.112:6080/arcgis/rest/services/AC/ACFacilities/FeatureServer/5",
				ArcGISFeatureLayer.MODE.ONDEMAND);
		// add secure layer to the map
		...
		//leader evacuation route
		 mTempLeaderrouteLayer = new ArcGISFeatureLayer(
		    		"http://10.0.0.112:6080/arcgis/rest/services/AC/ACFacilities/FeatureServer/3",
		 ...
		 mTemppolylinebarrierLayer = new ArcGISFeatureLayer(
		    		"http://10.0.0.112:6080/arcgis/rest/services/AC/ACFacilities/FeatureServer/1",
		        //"http://sampleserver5.arcgisonline.com/ArcGIS/rest/services/LocalGovernment/Recreation/FeatureServer/2",
		    		ArcGISFeatureLayer.MODE.ONDEMAND);
		...
		try {
			mRouteTask = RouteTask
					.createOnlineRouteTask(
							...
							"http://10.0.0.112:6080/arcgis/rest/services/AC/ACNetworkAnalysis/NAServer/Route",
							null);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		*/
        ...
        //btngotobroadcastform.setEnabled(false);
        btngotobroadcastform.setOnClickListener(gotobroadcastListener());
		...
		map.setOnLongPressListener(new OnLongPressListener() {
			...
			public boolean onLongPress(final float x, final float y) {
				...
				dialog = ProgressDialog.show(RoutingtoshelterActivity.this, "",
	 ...
	 public OnClickListener gotobroadcastListener() {
		    ...
		    return new OnClickListener() {
		      ...
		      public void onClick(View v) {
		    	  ...
		    	  try{
		            ...
		            if(v==btngotobroadcastform){  
		            	
		            	if(pushinfofromleader2followers==""){
		            		
		        			Toast.makeText(RoutingtoshelterActivity.this, "There is no route update",
		        					Toast.LENGTH_LONG).show();
		        			return;
		            	}		            	
		              intent.setClass(RoutingtoshelterActivity.this, LeaderBroadcastActivity.class); 
		              Bundle bundle=new Bundle();  		              		            
		              bundle.putString("info", pushinfofromleader2followers);
	...
	void updateUI() {
		dialog.dismiss();
		if (mResults == null) {
			Toast.makeText(RoutingtoshelterActivity.this, mException.toString(),
					Toast.LENGTH_LONG).show();
			return;
		}
		    ...
		    mTemproadsegmentLayer.selectFeatures(segmentquery, ArcGISFeatureLayer.SELECTION_METHOD.NEW, new CallbackListener<FeatureSet>() {
		      ...
		      public void onCallback(FeatureSet queryResults) {
		        ...
		        if (queryResults.getGraphics().length > 0) {	
		        	...
		        	String removeurl="http://10.0.0.112:8080/segmentleader.do?action=removeleadersbyleaderid&leaderid="+leaderid;
		    	 ...
		    	 for(int i=0;i<queryResults.getGraphics().length;i++){
		    		...
		    		String purl="http://10.0.0.112:8080/segmentleader.do?action=saveleadersegments&segmentid="+segmentid+"&leaderid="+leaderid;
			    ...
			    if(pushinfofromleader2followers!=null)
			    {
			    	btngotobroadcastform.setEnabled(true);
		   		...
		   		String removerouteurl="http://10.0.0.112:8080/leadershelterroute.do?action=removeroutebyleaderid"+
		   				"&leaderid="+leaderid;
	        	RegisterAsyncTask Registerremoveroute = new RegisterAsyncTask(removerouteurl);
	        	Registerremoveroute.execute("");
	        	
		   		String saverouteurl="http://10.0.0.112:8080/leadershelterroute.do?action=saveleadershelterroute"+
