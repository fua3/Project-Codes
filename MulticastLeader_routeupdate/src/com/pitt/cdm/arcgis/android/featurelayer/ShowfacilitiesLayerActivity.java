...
package com.pitt.cdm.arcgis.android.featurelayer;
...
public class ShowfacilitiesLayerActivity extends Activity {
	// ArcGIS elements
	MapView mMapView;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		featureServiceURL = this.getResources().getString(
				R.string.fs_communityfeatureserver_url);
		...
		// create the secure service URL
		featureServiceURL = this.getResources().getString(
				R.string.fs_riskregionfeatureserver_url);
		// create the secured layer with credentials
		secureFeatureLayer = new ArcGISFeatureLayer(featureServiceURL,
				MODE.ONDEMAND, creds);
		// add secure layer to the map
		...
		// create the secure service URL
		featureServiceURL = this.getResources().getString(
				R.string.fs_shelterfeatureserver_url);
		// create the secured layer with credentials
		secureFeatureLayer = new ArcGISFeatureLayer(featureServiceURL,
				MODE.ONDEMAND, creds);
		// add secure layer to the map
	...
	class QueryFeatureService extends AsyncTask<String, Void, FeatureSet> {
		...
		protected void onPreExecute() {
			dialog = ProgressDialog.show(ShowfacilitiesLayerActivity.this, "",
		...
		protected FeatureSet doInBackground(String... params) {
			...
			try {
				...
				creds.setUserAccount(
						ShowfacilitiesLayerActivity.this.getResources().getString(
		...
		protected void onPostExecute(FeatureSet featureSet) {
			...
			alertDialogBuilder = new AlertDialog.Builder(
					ShowfacilitiesLayerActivity.this);
